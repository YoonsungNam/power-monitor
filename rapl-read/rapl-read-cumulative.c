...
static double max_pkg_energy_value = 262143328850;
static double max_dram_energy_value = 65712999613;
...
static int rapl_sysfs(int core) {
	...
	printf("\tProcess is stoped by SIGSTOP...\n\n");
	int my_pid = getpid();
	kill(my_pid, SIGSTOP);
	//sleep(1);
	...
	for(j=0;j<total_packages;j++) {
		...
		for(i=0;i<NUM_RAPL_DOMAINS;i++) {
			if (valid[j][i]) {
				if (j==0){	//in case of pkg power
					if ((double)after[j][i] < (double)before[j][i]){
						//The case of overflow
						printf("\t\t%s\t: %lfJ\n",event_names[j][i],
							(max_pkg_energy_value-(double)before[j][i]+(double)after[j][i])/1000000.0);
					}
					else{
						printf("\t\t%s\t: %lfJ\n",event_names[j][i],
				...
				else if (j==1){	//in case of dram power
					if ((double)after[j][i] < (double)before[j][i]){
						//The case of overflow
						printf("\t\t%s\t: %lfJ\n",event_names[j][i],
							(max_dram_energy_value-(double)before[j][i]+(double)after[j][i])/1000000.0);
					}
					else{
						printf("\t\t%s\t: %lfJ\n",event_names[j][i],
							((double)after[j][i]-(double)before[j][i])/1000000.0);
